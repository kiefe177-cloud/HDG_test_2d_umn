// Tests for tfi_elliptic_orthogonal.
//
// tfi_elliptic_orthogonal is a Winslow/TTM elliptic smoother with
// Steger-Sorenson wall orthogonality enforcement (no marching, no wall
// sliding — that's full_orthogonal's territory).
//
// Test A: Pure smoothing on a noisy unit square (orthogonality DISABLED).
//   Exercises only the Winslow/TTM SOR loop. The solver should recover
//   the clean unit square from a noisy interior.
//
...
//   algebraic blend. Verifies that wall skewness drops and no cell folds.
...
int test_pure_smoothing() {
    ...
    // Add noise to interior only (boundaries untouched). Distinct phases on
    // X and Y so the noise isn't purely diagonal.
    for (int i = 1; i < N - 1; ++i) {
        for (int j = 1; j < N - 1; ++j) {
    ...
    save_csv(std::string(OUTPUT_DIR_TEST) + "/testA_ortho_noisy_x.csv", X);
    save_csv(std::string(OUTPUT_DIR_TEST) + "/testA_ortho_noisy_y.csv", Y);
    ...
    auto result = tfi_elliptic_orthogonal(
        X, Y, phi, psi, N,
        /*n_elliptic_iter=*/20000, /*sor_omega=*/1.6, /*tol=*/1e-10, opts);

    save_csv(std::string(OUTPUT_DIR_TEST) + "/testA_ortho_smooth_x.csv", result.X_ellip_grid);
    save_csv(std::string(OUTPUT_DIR_TEST) + "/testA_ortho_smooth_y.csv", result.Y_ellip_grid);
...
int test_orthogonality() {
    ...
    save_csv(std::string(OUTPUT_DIR_TEST) + "/testB_ortho_initial_x.csv", X);
    save_csv(std::string(OUTPUT_DIR_TEST) + "/testB_ortho_initial_y.csv", Y);
    ...
    save_csv(std::string(OUTPUT_DIR_TEST) + "/testB_ortho_final_x.csv", result.X_ellip_grid);
    save_csv(std::string(OUTPUT_DIR_TEST) + "/testB_ortho_final_y.csv", result.Y_ellip_grid);

    bool bnd_ok = boundaries_preserved(
        X_ref, Y_ref, result.X_ellip_grid, result.Y_ellip_grid, 1e-12);
    std::cout << "  Boundaries preserved: " << (bnd_ok ? "yes" : "NO") << "\n";
    if (!bnd_ok) {
        std::cerr << "  [FAIL] boundary moved (this solver should hold all four boundaries)\n";
        return 1;
    }
